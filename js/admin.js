$(document).ready(function(){
  var showRecords = $("#showRecords").DataTable({
    ...
    columns: [{
        ...
        "orderable": false,
        "data": null,
        "defaultContent": "<button class='btn btn-primary view_btn'>View</button>&nbsp;<button class='btn btn-success check_btn'>Approve</button>&nbsp;<button class='btn btn-danger check_btn'>Disapprove</button>"
    }, {
  ...
  $("#showRecords").on("click",'.view_btn',function()
  {
    ...
    $.get("ajax/showCensusForm2.php", {application_number : data["application_number"]}, function(data) {
      $("#secondPage").find("#member_div").html(data);
    });
    $.post("ajax/showDatas.php", {application_number : data["application_number"]}, function(data) {
